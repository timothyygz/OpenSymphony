...
describe("stepTracker", () => {
  test("happy path: connection ok, skip ownership transfer", async () => {
    const { deps, enqueue } = createMockDeps();
    // appId, appSecret (from group), phone (empty)
    enqueue("cli_test_app", "test_secret", "");
  ...
  test("with ownership transfer", async () => {
    const { deps, enqueue } = createMockDeps({
      lookupUserByMobile: async () => "ou_transferred_user",
      transferOwnership: async () => {},
    });
    enqueue("cli_app", "secret", "13800138000");
  ...
  test("returns null on createApp failure", async () => {
    const { deps, enqueue } = createMockDeps({
      createApp: async () => { throw new Error("API error"); },
    });
    enqueue("cli_app", "secret");

    const result = await stepTracker(deps);
    expect(result).toBeNull();
  });

  test("returns null on createTable failure", async () => {
    const { deps, enqueue } = createMockDeps({
  ...
  test("returns null when user cancels on appId/appSecret", async () => {
    const { deps, enqueue } = createMockDeps();
    // group resolves each prompt; first one returns CANCEL
    enqueue(CANCEL);
