...
describe("initSources", () => {
  ...
  it("creates worktree from local repo", async () => {
    ...
    expect(existsSync(resolve(workspacePath, "worktree-copy", "readme.md"))).toBe(true);
  });
  ...
  it("cleans up worktree sources", async () => {
    ...
    const { stdout } = Bun.spawnSync(["git", "-C", localRepo, "worktree", "list"]);
  ...
  it("rolls back on partial failure", async () => {
    ...
    const sources = [
      ...
      { type: "git-clone" as const, url: "https://invalid-url-that-does-not-exist.example.com/repo.git", path: "fail", depth: 1 },
    ];

    await expect(initSources(sources, workspacePath, tempDir)).rejects.toThrow();
...
describe("hashSources", () => {
  it("produces consistent hash for same sources", () => {
    const sources = [
      { type: "git-clone" as const, url: "https://example.com/repo.git", path: "repo", depth: 1 },
    ];
    expect(hashSources(sources)).toBe(hashSources(sources));
  });

  it("produces different hash for different sources", () => {
    const a = [{ type: "git-clone" as const, url: "https://a.com/repo.git", path: "repo", depth: 1 }];
    const b = [{ type: "git-clone" as const, url: "https://b.com/repo.git", path: "repo", depth: 1 }];
